...
function RoleMenu({ visible, onCancel, roleId, onSave }: RoleMenuProps) {
  const [treeData, setTreeData] = useState<DataNode[]>([]);
  const [getDataLoading, setGetDataLoading] = useState(false);
  const [checkedKeys, setCheckedKeys] = useState<string[]>([]);
  const [saveLoading, setSaveLoading] = useState(false);
  const [selectType, setSelectType] = useState<
    'allChildren' | 'current' | 'firstChildren'
  >('allChildren');
  ...
  const onCheck = (
    _: Key[] | { checked: Key[]; halfChecked: Key[] },
    { checked, node }: { checked: boolean; node: CheckNode }
  ) => {
  ...
  const getData = async () => {
    ...
    const [error, data] = await to(roleService.getAllMenus());
  ...
  const getCheckedKeys = async () => {
    ...
    const [error, data] = await to(roleService.getRoleMenus(roleId));
