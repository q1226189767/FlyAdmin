...
function NewAndEditMenuForm({
  ...
  const { data: apiData, run: getApiList } = useRequest(
    menuService.getApiList,
    { manual: true }
  );
  const { runAsync: getAllocApis } = useRequest(menuService.getAllocApis, {
    manual: true,
  });
  const { run: addMenu, loading: createLoading } = useRequest(
    menuService.addMenu,
    {
      manual: true,
      onSuccess: () => {
  ...
  const { run: updateMenu, loading: updateLoading } = useRequest(
  ...
  async function setDefaultFormValues() {
    if (editData?.id) {
      ...
      if (!error) {
        form.setFieldValue(
          'apis',
          allocApis.map((api) => [api.method, api.path].join('~'))
        );
      }
    } else if (curRecord) {
      form.setFieldsValue({
  ...
  const renderButtonForm = () => {
    return (
      <>
        ...
        <Form.Item label={t('LxiWbxsx' /* 绑定接口 */)} name="apis">
